...
# 处理手绘图片
def process_painting_image(painting_image_path):
    """

    Args:
        painting_image_path: String, 图片路径

    Returns:
        返回lap数据以及process_image_path
    """
    tool_log.debug("process_original_image", painting_image_path)

    lap, tau, process_image_path = function.process_painting_image(painting_image_path)
    # 修正输出的图片
    cropped_image = tool_image.crop_image_by_path(process_image_path, 101, 221, 534, 409)
    tool_image.save_image_to_dir(cropped_image, process_image_path)
    lap = tool_formula.set_round(lap, 4)
    tau = tool_formula.set_round(tau, 4)
    # 设置lap返回的精确度
